    ...
    public async Task FunctionHandler(S3Event evnt, ILambdaContext context)
    {
        ...
        foreach (var record in eventRecords)
        {
            ...
            try
            {
                ...
                string bucketNameProcessed = $"{bucketName}-processed";
                
                // create processed bucket if not exists
                var isCreated = await AwsS3BucketHandler.CreateBucketIfNotExistAsync(context, S3Client, bucketNameProcessed);
                
                // copy file to processed bucket
                var isCopied = await AwsS3BucketHandler.CopyS3FileAsync(context, S3Client, bucketName, 
                    s3Event.Object.Key, bucketNameProcessed, s3Event.Object.Key);
                
                // delete file from original bucket
                var isDeleted = await AwsS3BucketHandler.DeleteS3FileAsync(context, S3Client, bucketName, s3Event.Object.Key);
