    ...
    ICurrentUser currentUser) : IUserService
{
    ...
    {
        return await _userRepository.GetByIdAsync(id) is not { } user
            ? null
    ...
    public async Task<UserDto?> UpdateAsync(Guid id, UserDto entity)
    {
        ...
        // Viktig å sjekke om innlogget bruker har rettigheter til å oppdatere brukeren
        if (!string.Equals(id.ToString(), _currentUser.UserId) && !_currentUser.IsAdmin)
        {   
    ...
    public async Task<UserDto?> RegisterAsync(UserRegistrationDto userRegistrationDto)
    {
        var existingUser = await _userRepository.FindAsync( u=> 
            u.UserName == userRegistrationDto.UserName || u.Email == userRegistrationDto.Email);
